...
sudo ffmpeg -stream_loop -1 -nostdin -re -i BigBuckBunny.mp4 -vf "drawtext=x=0:y=8:box=1:fontcolor=yellow:fontsize=50:boxcolor=black:expansion=strftime:basetime=$(date +%s -d'00:00:00')000000:text='$newline Start\:%Y-%m-%d %H\\:%M\\:%S Cfg\: $cfg $newline'"  -f v4l2 /dev/video0 & PULSE_SINK=virtual_speaker ffmpeg -stream_loop -1 -i BigBuckBunny.mp4 -f pulse "stream name"
kill $!
